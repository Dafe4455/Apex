    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ud-action-btn.orange { background: var(--orange-l); color: var(--orange); border-color: var(--orange-m); }
        .ud-action-btn.red    { background: var(--red-l);    color: var(--red);    border-color: #e8b8b8; }
        .ud-action-btn.green  { background: var(--green-l);  color: var(--green);  border-color: #a8d8b8; }
        .ud-action-btn.gold   { background: var(--gold-l);   color: var(--gold);   border-color: #e8d48a; }
        ...
        .ud-form-title {
          ...
          justify-content: space-between; font-family: var(--sans);
        }
        .ud-field { margin-bottom: 14px; }
        .ud-field-label {
          display: block; font-size: 0.58rem; font-weight: 600;
          color: var(--ink-faint); text-transform: uppercase;
          letter-spacing: 0.08em; margin-bottom: 6px; font-family: var(--sans);
        }
        .ud-input {
        ...
        .ud-kyc-pill {
          ...
          font-weight: 600; cursor: pointer; transition: all 0.12s; color: var(--ink-dim);
        }
        ...
        .ud-modal {
          ...
          box-shadow: 0 20px 60px rgba(28,26,23,0.2); overflow: hidden;
        }
        .ud-modal.lg { max-width: 520px; max-height: 88vh; display: flex; flex-direction: column; }
        .ud-modal-head {
          padding: 16px 20px; border-bottom: 1px solid var(--bg-2);
        ...
        .ud-modal-close:hover { background: var(--bg-2); color: var(--ink); }
        .ud-modal-body { padding: 20px; }
        .ud-modal-scroll { overflow-y: auto; flex: 1; }

        /* balance operation cards */
        .ud-op-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
        .ud-op-card {
          text-align: left; padding: 10px 12px; border-radius: 10px;
          border: 1.5px solid var(--bg-2); background: var(--bg-1);
          cursor: pointer; transition: all 0.12s; font-family: var(--sans);
        }
        .ud-op-card:hover { border-color: var(--bg-3); }
        .ud-op-card.sel-credit { background: var(--green-l); border-color: #a8d8b8; }
        .ud-op-card.sel-debit  { background: var(--red-l);   border-color: #e8b8b8; }
        ...
        .ud-preview {
          font-size: 0.65rem; font-family: var(--mono); color: var(--ink-faint);
        ...
        .ud-mbtn.ghost  { background: var(--bg-2);    color: var(--ink-dim); }
        .ud-mbtn.credit { background: var(--green-l); color: var(--green); border: 1px solid #a8d8b8; font-weight: 700; }
        .ud-mbtn.debit  { background: var(--red-l);   color: var(--red);   border: 1px solid #e8b8b8; font-weight: 700; }
        .ud-mbtn.danger { background: var(--red-l);   color: var(--red);   border: 1px solid #e8b8b8; font-weight: 700; }
        .ud-mbtn.send   { background: var(--orange);  color: #fff;          font-weight: 700; }
        ...
        .ud-email-log-subject { font-size: 0.75rem; font-weight: 600; color: var(--ink); margin-bottom: 3px; font-family: var(--sans); }
        ...
        .ud-email-status.ok  { background: var(--green-l); color: var(--green); border: 1px solid #a8d8b8; }
        .ud-email-status.err { background: var(--red-l);   color: var(--red);   border: 1px solid #e8b8b8; }
      ...
      <div className="ud-wrap">
        ...
        {!loading && error && (
          <div className="ud-center">
            ...
            <p style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--ink)', fontFamily: 'var(--sans)' }}>{error}</p>
        ...
        {!loading && user && !editing && (
          <>
            ...
            <div className="ud-actions">
              <button className="ud-action-btn orange" onClick={() => setShowBalanceModal(true)}>
                <Wallet size={13} /> Adjust Balance
              </button>
              <button className="ud-action-btn gold" onClick={openEmailModal}>
                <Mail size={13} /> Send Email
              </button>
              <button className="ud-action-btn red" onClick={() => setShowResetModal(true)}>
            ...
            <div className="ud-info-grid">
              ...
              <div className="ud-info-row">
                <div className="ud-info-ico"><Phone size={15} /></div>
                <div style={{ flex: 1 }}>
                  <p className="ud-info-label">Phone</p>
                  <p className={`ud-info-val${!user.phone ? ' faint' : ''}`}>{user.phone || 'Not provided'}</p>
                </div>
              </div>

              <div className="ud-info-row">
                <div className="ud-info-ico"><Globe size={15} /></div>
                <div style={{ flex: 1 }}>
                  <p className="ud-info-label">Country</p>
                  <p className={`ud-info-val${!user.country ? ' faint' : ''}`}>{user.country || 'Not provided'}</p>
                </div>
              </div>
                ...
                <label className="ud-field-label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  {!emailEditable && (
                    <button onClick={() => setEmailEditable(true)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--orange)', display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.58rem', fontWeight: 600, fontFamily: 'var(--sans)' }}>
                      <Pencil size={9} /> Change
                    </button>
                  )}
                </label>
                <input className="ud-input" type="email" value={email}
          ...
          <div className="ud-modal">
            ...
            <div className="ud-modal-body">
              ...
              <div className="ud-field">
                ...
                <div className="ud-amount-wrap">
                  ...
                  <input type="number" placeholder="0.00" value={amount}
                    ...
                    className="ud-input ud-amount-input" />
                </div>
                {amount && !isNaN(parseFloat(amount)) && (
                  <p className="ud-preview">
                    ...
                    <span className={isCredit ? 'credit' : 'debit'}>
                      ${(isCredit
                        ? (user?.portfolioBalance ?? 0) + parseFloat(amount)
                        : (user?.portfolioBalance ?? 0) - parseFloat(amount)
              ...
              <div className="ud-field" style={{ marginBottom: 0 }}>
                ...
                <input type="text" placeholder="e.g. BTC/USDT trade — closed at profit"
                  value={balanceNote} onChange={e => setBalanceNote(e.target.value)}
                  className="ud-input" />
              </div>
              ...
              <div className="ud-modal-btns">
                ...
                <button
                  className={`ud-mbtn ${isCredit ? 'credit' : 'debit'}`}
        ...
        <div className="ud-overlay">
          <div className="ud-modal lg">
            ...
            <div className="ud-modal-scroll">
              <div className="ud-modal-body" style={{ borderBottom: '1px solid var(--bg-2)' }}>
                <p style={{ fontSize: '0.65rem', fontFamily: 'var(--mono)', color: 'var(--ink-faint)', marginBottom: 14 }}>
                  ...
                  <span style={{ color: 'var(--bg-3)' }}>&lt;{user?.email}&gt;</span>
                </p>

                <div className="ud-field">
                  <label className="ud-field-label">Subject</label>
                  <input type="text" placeholder="e.g. Account Verification Required"
                ...
                <div className="ud-modal-btns">
                  ...
                  <button className="ud-mbtn send"
                    disabled={sendingEmail || !emailSubject || !emailMessage}
                    onClick={handleSendEmail}>
                    {sendingEmail
                      ? <><Loader2 size={13} className="ud-spin" /> Sending…</>
              ...
              <div className="ud-modal-body">
                ...
                {loadingLogs ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--ink-faint)', fontSize: '0.7rem', fontFamily: 'var(--mono)' }}>
