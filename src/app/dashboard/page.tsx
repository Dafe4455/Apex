...
export default function DashboardPage() {
  ...
  // Navigate to trade page with asset + action pre-selected
  const handleTrade = (symbol: string, action: 'BUY' | 'SELL') => {
    const tradeSymbol = ['BTC', 'ETH', 'SOL', 'BNB'].includes(symbol)
      ? symbol + 'USD'
      : symbol;
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        /* FIXED bottom metrics — 2 col instead of 4 */
        .bottom-metrics{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:0 18px 10px;}
        .bm-card{background:var(--card);border:1px solid var(--bg-2);border-radius:12px;padding:14px 16px;}
        .bm-lbl{font-size:0.58rem;font-weight:600;color:var(--ink-faint);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:8px;}
        .bm-val{font-size:1.3rem;font-weight:700;color:var(--ink);letter-spacing:-0.02em;line-height:1;margin-bottom:4px;}
        .bm-sub{font-size:0.62rem;font-weight:300;color:var(--ink-faint);line-height:1.5;}
        .bm-dot{font-size:0.6rem;color:var(--green);}
        .bm-dot.red{color:var(--red);}
        .bm-notif{font-size:0.6rem;font-weight:400;color:var(--ink-dim);line-height:1.5;margin-bottom:4px;padding-left:2px;}
        .bm-notif:last-child{margin-bottom:0;}
        ...
        .btn-sell:hover{background:var(--red-l);color:var(--red);border-color:var(--red);}
      ...
      <div className="dash-wrap">
        ...
        {/* BOTTOM METRICS — 2 col, properly spaced */}
              ...
              : markets.map(a => (
                <div key={a.id} className="asset-row">
                  ...
                  <div className="trade-btns">
                    <button className="btn-buy" onClick={() => handleTrade(a.symbol, 'BUY')}>Buy</button>
                    <button className="btn-sell" onClick={() => handleTrade(a.symbol, 'SELL')}>Sell</button>
                  </div>
                </div>
              ))}
          </div>
